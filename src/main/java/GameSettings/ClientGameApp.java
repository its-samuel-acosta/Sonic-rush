...
public class ClientGameApp extends GameApplication {
    ...
    public GameLogic gameLogic; // Declarar pero no inicializar aquí

    // Temporizador del juego
    private TimerAction gameTimerAction;
    private double timeLeft = 180.0; // 180 segundos = 3 minutos
    ...
    protected void initGame() {
        ...
        // Toda la inicialización de UI debe hacerse en el hilo de la aplicación FX
        getExecutor().startAsyncFX(() -> {
            gameLogic = new GameLogic(); // Inicializar GameLogic aquí
            gameLogic.init(); // Y llamarlo para inicializar sus elementos de UI

            showMainMenu(); // Mostrar el menú principal
        });

        // El spawn de Eggman se ha movido a startNetworkAndGame()
    }
    ...
    private void showMainMenu() {
        // Este método ya está envuelto en getExecutor().startAsyncFX()
        // cuando se llama desde initGame(), y también en resetGameAndReturnToMenu().
        // No necesita un envoltorio adicional aquí.
   ...
   @Override
    ...
    private void resetGameAndReturnToMenu() {
        ...
        getGameTimer().runOnceAfter(() -> {
            ...
            if (gameTimerAction != null) {
                gameTimerAction.expire();
                gameTimerAction = null; // Limpiar la referencia
